...
function forward26(p2) {
  ...
  if (this.sphere) {
    ...
    if (Math.abs(this.sin_p12 - 1) <= EPSLN) {
      ...
      azi1 = vars.azi1;
      p2.x = this.x0 + vars.s12 * Math.sin(azi1);
      p2.y = this.y0 + vars.s12 * Math.cos(azi1);
      return p2;
    }
  }
}
...
function forward34(p2) {
  return this.projectionType.forward(this, p2);
}
function inverse34(p2) {
...
var parseZip = async function(buffer, whiteList) {
  ...
  for (key in zip) {
    ...
    if (key.slice(-4).toLowerCase() === ".shp") {
      ...
      names38.push(key.slice(0, -3) + key.slice(-3).toLowerCase());
  ...
  const geojson = names38.map(function(name) {
    let parsed, dbf;
